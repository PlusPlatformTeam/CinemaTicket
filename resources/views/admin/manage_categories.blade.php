...
@section('content')
    <section class="p-4">
        <div class="mb-6">
            <button data-modal-target="create-category-modal" data-modal-toggle="create-category-modal" type="button"
            ...
            <table class="w-full text-sm text-left text-gray-500">
                ...
                <tbody>
                    @foreach ($categories as $category)
                        <tr id="category-{{$category->id}}" class="bg-white border-b  hover:bg-gray-50 ">
                            ...
                            <td class="py-3 text-center">
                                <a data-tooltip-target="tooltip-edit-category{{ $category->id }}" href="#"
                                    class="font-medium text-lg text-blue-600 hover:underline mx-2"><i
                                        class="fa-regular fa-pen-to-square"></i></a>
                                <div id="tooltip-edit-category{{ $category->id }}" role="tooltip"
                                    class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-sm opacity-0 tooltip">
                                    ویرایش
                                    <div class="tooltip-arrow" data-popper-arrow></div>
                                </div>

                                <button data-modal-target="delete-modal-category{{ $category->id }}"
                                    data-modal-toggle="delete-modal-category{{ $category->id }}" type="button"
                                    data-tooltip-target="tooltip-delete-category{{ $category->id }}" href="#"
                                    class="font-medium text-lg text-red-600 hover:underline mx-2"><i
                                        class="fa-regular fa-trash-can"></i></button>
                                <div id="tooltip-delete-category{{ $category->id }}" role="tooltip"
                                    class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-sm opacity-0 tooltip">
                                    حذف
                                    <div class="tooltip-arrow" data-popper-arrow></div>
                                </div>
                        ...
                        <div id="delete-modal-category{{ $category->id }}" tabindex="-1"
                                ...
                                <div class="relative bg-white rounded-lg shadow">
                                    ...
                                    <div class="flex items-center justify-between p-5 border-b rounded-t">
                                        ...
                                        <button type="button"
                                            ...
                                            data-modal-hide="delete-modal-category{{ $category->id }}">
                                    ...
                                    <div class="p-6 space-y-6">
                                        <p class="text-base leading-relaxed text-gray-500 ">
                                            آیا از حذف کردن ژانر <b>{{ $category->name }}</b> مطمین هستید ؟؟
                                        </p>
                                    </div>
                                    <!-- Modal footer -->
                                    <div class="flex items-center p-6 space-x-2 border-t border-gray-200 rounded-b">
                                        <button onclick="deleteCategory('{{ $category->id }}')" data-modal-hide="delete-modal-category{{ $category->id }}" type="button"
                                        ...
                                        <button data-modal-hide="delete-modal-category{{ $category->id }}" type="button"
    ...
    <div id="create-category-modal" tabindex="-1"
        class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
        <div class="relative w-full max-w-md max-h-full">
            <!-- Modal content -->
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="p-6 space-y-6">
                    <form id="create-category-form" enctype="multipart/form-data"
                        action="{{ route('admin.manage.category.create') }}" method="POST">
                        @csrf
                ...
                <div
                    ...
                    <button data-modal-hide="create-category-modal" type="button"
    ...
    <script>
        function deleteCategory(categoryID) {
            $.ajax({
                url: "{{ route('admin.manage.category.create') }}",
                type: "DELETE",
                headers: {
