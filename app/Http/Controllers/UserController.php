...
class UserController extends Controller
{
    ...
    public function profileUpdate(Request $request)
    {
        ...
        if ($name != $user->name || $email != $user->email || $mobile != $user->mobile || $birthday != $user->birthday) {
            ...
            return redirect()->back()->with('success', 'پروفایل کاربر با موفقیت آپدیت شد');
        } else {
    ...
    public function Delete(Request $request)
    {
        ...
        if ($user) {
            ...
            return response()->json([
                'message' => 'کاربر با موفقیت پاک شد',
            ]);
        } else {
            return response()->json([
    ...
    {
        return view('admin.manage_users', ['users' => User::where('type', '<>', User::ADMIN_TYPE)->paginate(2)]);
    }
}
